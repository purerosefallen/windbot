...
- `DeckCode`: A Base64-encoded update-deck payload. When present, it takes precedence over `DeckFile` for the cards sent to the room.
...
- `RealIP`: An optional IPv4 address sent to servers that support the external-address extension.
...
- `AutoQuit`: Set to `True` to leave the room automatically when the opponent leaves. The default is `False`.
...
Query parameter names are lowercase. Supported parameters are `name`, `deck`, `deckfile`, `deckcode`, `host`, `port`, `dialog`, `version`, `password`, `hand`, `debug`, `chat`, and `realip`. The `name` and `host` parameters are required, along with `port` unless the host URL already contains the connection endpoint. An unknown explicit `deck` is rejected.

In addition to ordinary HTTP requests that start outbound TCP or WebSocket clients, this branch accepts a WebSocket upgrade and uses the accepted connection as a reverse-WebSocket YGOPro client.
...
- Supports TCP, outbound WebSocket, and reverse-WebSocket connections used by KoishiPro services.
- Supports `Group.SelectUnselect` behavior used by KoishiPro and EDOPro-compatible servers.
